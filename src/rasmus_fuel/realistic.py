...
----------
    ...
    coeff_water_drag = 0.5 * physics_param["surface_water_density"] * vessel_param["water_drag_coefficient"] * \
    ...
    wind_dir_rel = wind_dir - course_og +  np.pi

    u_wind_rel = wind_speed * np.cos(wind_dir_rel) - u_ship_og
    v_wind_rel = wind_speed * np.sin(wind_dir_rel) - v_ship_og
    
    coeff_wind_drag = 0.5 * physics_param["air_mass_density"] * physics_param["wind_resistance_coefficient"] * \
                      vessel_param["vessel_supersurface_area"]
    
    coeff_wave_drag = 20. * (vessel_param["waterline_width"] / vessel_param["waterline_length"]) ** (-1.2) * \
                      (1 / vessel_param["waterline_length"]) ** 0.62 \
                      / vessel_param["total_propulsive_efficiency"] / physics_param["reference_froede_number"] * \
                      physics_param["spectral_average"] * physics_param["surface_water_density"] * \
                      vessel_param["waterline_width"] ** 2 * (physics_param["acceleration_gravity"]/vessel_param["waterline_length"]**3)**0.05 \
                      * vessel_param["vessel_draught"] ** 0.62 * 0.25
