...
def test_realistic_power_maintain_sog_no_drag():
    ...
    course_ship_og = np.random.normal(size=(10,))
...
def test_power_maintain_sog_errors():
    ...
    with pytest.raises(ValueError) as execinfo:
        ...
        u_wind=1, v_wind=2, w_wave_hight=0, course_ship_og = 0)
...
def test_power_maintain_sog_kwargs_ignored():
    ...
    np.testing.assert_equal(
        power_maintain_sog(
            u_ship_og=1, v_ship_og=1, u_current=0, v_current=0, u_wind=1, v_wind=2, w_wave_hight=0, course_ship_og = 0,
